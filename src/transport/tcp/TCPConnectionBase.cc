...
TCPStateVariables::TCPStateVariables()
{
    ...
    rcv_nxt = 0;
    rcv_wnd = 0; // will be set from configureStateVariables() and probably reset after receiving ACKs from data receiver
    rcv_up = 0;
    irs = 0;
...
TCPConnection::TCPConnection()
{
    ...
    sndWndVector = rcvWndVector = sndNxtVector = sndAckVector = rcvSeqVector = rcvAckVector = unackedVector =
    sndGapsVector = dupAcksVector = sndSacksVector = rcvSacksVector = rcvOooSegVector =
    tcpRcvQueueBytesVector = tcpRcvQueueDropsVector = NULL;
}
...
TCPConnection::TCPConnection(TCP *_mod, int _appGateIndex, int _connId)
{
    ...
    if (getTcpMain()->recordStatistics)
    {
        ...
        sndGapsVector = new cOutVector("snd_gaps");
